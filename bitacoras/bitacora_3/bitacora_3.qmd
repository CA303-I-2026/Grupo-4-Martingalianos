...
A partir de esta cantidad se define

$$
\widehat{\pi}_{x,t}^{(c)}
=
P(J=c \mid \text{muerte en el grupo } x)
=
\frac{
\widehat{q}_{x,t}^{(c)}
}{
...
Esta cantidad debe de interpretarse como una probabilidad condicional: Dado que ocurre un fallecimiento en la celda, $\widehat{\pi}_{x,t}^{(c)}$ representa la proporción atribuible a la causa $c$. Por tanto, mientras $\widehat{q}_{x,t}^{(c)}$ mide el nivel absoluto del riesgo, $\widehat{\pi}_{x,t}^{(c)}$ describe la estructura causal de la mortalidad dentro de quienes fallecen.

Para una celda fija, las cantidades $\widehat{\pi}_{x,t}^{(c)}$ forman una distribución de probabilidad sobre las causas, ya que
...
Esta propiedad justifica el uso de gráficos de composición causal, similares en espíritu a los análisis de distribución de causas de muerte por grupo de edad, utilizados por Goerlich (2012). Sin embargo, para mantener la legibilidad, se propone mostrar únicamente algunas causas seleccionadas. En estos casos, el denominador de $\widehat{\pi}_{x,t}^{(c)}$ se mantendría como la suma de todas las causas, aunque solo se visualicen algunas. Por esta razón, cuando se presenten únicamente causas seleccionadas, las barras visibles no necesariamente sumarían $100\%$.
