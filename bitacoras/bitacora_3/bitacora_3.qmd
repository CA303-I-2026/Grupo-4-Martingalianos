...
- Análisis de decremento por causa q.
...
Un punto importante en la lectura de @fig-composicion-causal-por-edades-nicaragua es que las barras no acumulan necesariamente el $100\%$ de la composición causal. Esto ocurre porque la visualización incluye únicamente las cuatro causas con mayor participación relativa global, junto con la causa perinatal. Si se denota por $\mathcal C_{\mathrm{sel}}$ el conjunto de causas seleccionadas en esa figura, entonces la altura total de cada barra representa
...
- Concentración de composición causal H
...
## 1.3 Contrucción de fichas de resultados

## Enfoque clásico:

## Enfoque clásico: composición causal
...
Esta comparación permite formular una lectura útil para los resultados propios, aunque con una diferencia metodológica importante. En el caso de Nicaragua, la composición causal sugiere que las enfermedades del sistema circulatorio adquieren progresivamente mayor peso relativo en edades avanzadas. Sin embargo, esta lectura corresponde a una composición local por grupo etario, no a una probabilidad acumulada de fallecer eventualmente por dicha causa. Por ello, el paralelismo con Goerlich Gisbert (2012) debe entenderse como conceptual: ambos análisis estudian cómo cambia el peso de las causas dentro de una estructura etaria de mortalidad, pero no estiman exactamente la misma cantidad.

La gráfica radar de probabilidad local de decremento por sistema circulatorio debe analizarse separadamente, porque no representa composición causal $\widehat{\pi}_{x,t,p,s}^{(c)}$, sino probabilidad local absoluta ${}_n\widehat q_{x,t,p,s}^{(c)}$. En esa visualización, para hombres de 65--69 años, Belice muestra un aumento entre 2015 y 2018 en la probabilidad local de decremento por enfermedades del sistema circulatorio. Esto va en dirección opuesta a la mejora de supervivencia frente a causas circulatorias descrita por Goerlich Gisbert (2012) para España en el largo plazo. No obstante, esta diferencia no debe interpretarse automáticamente como un deterioro estructural de la supervivencia en Belice. Dado que se trata de una comparación local, limitada a un grupo etario, un sexo y dos años calendario, el aumento puede reflejar variación transitoria, ruido propio de poblaciones pequeñas o cambios específicos en el registro de defunciones y exposición. Por esta razón, el radar será analizado en una ficha posterior centrada en ${}_n\widehat q_{x,t,p,s}^{(c)}$, mientras que la presente ficha se mantiene enfocada en la composición causal $\widehat{\pi}_{x,t,p,s}^{(c)}$.

#### Implicación para el siguiente paso.

El siguiente paso consiste en vincular esta lectura de composición causal con el índice de concentración causal,
...
Este índice permitirá evaluar de forma más sintética si la composición causal de un país y grupo etario está dominada por pocas causas o si se encuentra distribuida entre varias. En particular, el caso de Nicaragua sugiere que el análisis de $H$ puede ser útil para confirmar si la aparente fragmentación observada en la visualización completa se traduce en menores niveles de concentración causal respecto a Costa Rica. Asimismo, conviene contrastar estos resultados con las probabilidades absolutas ${}_n\widehat{q}_{x,t,p,s}^{(c)}$, para distinguir entre cambios en la participación relativa y cambios reales en la magnitud del riesgo de decremento.

## 1.3 Ordenamiento de los elementos del reporte.
